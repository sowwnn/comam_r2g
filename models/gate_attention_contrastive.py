...
class GatedAttentionUnit(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Feed forward song song (nếu cần)
        self.ff = nn.Sequential(
            nn.Linear(dim, ff_mult * dim * 2, bias=False),
            SwiGLU(),
    ...
    def forward(self, x, context):
        ...
        # tính toán gate (phần quan trọng của GAU)
        g = self.gate_proj(x)
        ...
        # apply gating - nhân với gate để lọc thông tin
...
class GateAttentionContrastive(nn.Module):
    def __init__(
        self,
        *,
        dim,
        ...
        self.img_attn_pool = GatedAttentionUnit(dim=dim, context_dim=image_dim, dim_head=dim_head, heads=heads, norm_context=True, parallel_ff=False)
        ...
        for ind in range(multimodal_depth):
            self.multimodal_layers.append(nn.ModuleList([
                Residual(ParallelTransformerBlock(dim=dim, dim_head=dim_head, heads=heads, ff_mult=ff_mult)),
                Residual(GatedAttentionUnit(dim=dim, dim_head=dim_head, heads=heads, ff_mult=ff_mult, parallel_ff=True))
            ]))
    ...
    def forward(
        ...
        # np.save('visualization/text_tokens.npy', text_tokens.cpu().numpy())
        # np.save('visualization/image_tokens.npy', image_tokens.cpu().numpy())
        ...
        for attn_ff, attn in self.multimodal_layers:
            text_tokens = attn_ff(text_tokens)
            text_tokens = attn(text_tokens, image_tokens)
